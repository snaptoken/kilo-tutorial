...
char *MARKDOWN_HL_extensions[] = { ".md", ".markdown", NULL };
...
void editorUpdateSyntax(erow *row) {
  ...
  int mcs_len = strlen(mcs);
  int mce_len = strlen(mce);
  ...
  while (*p) {
    ...
    if (mcs[0] != '\0' && mce[0] != '\0') {
      if (in_comment) {
        ...
        if (i + mce_len <= row->rsize && !memcmp(p, mce, mce_len)) {
      ...
      } else if (i + mcs_len <= row->rsize && !memcmp(p, mcs, mcs_len)) {
