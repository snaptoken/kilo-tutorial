...
Currently, our `die()` function isn't suitable for a text editor. The cursor
could be anywhere when we call `die()` later, and then the error message would
be printed in a random place on the screen, and a bunch of text would be left
still on the screen after our program exited. So let's clear the screen when
our program exits, and reposition the cursor back to the top-left corner.

{{clear-screen-atexit}}

`write()` and `STDOUT_FILENO` come from `<unistd.h>`.

This uses escape sequences to tell the terminal to move the cursor and clear
the screen. All escape sequences start with the escape character (`27`, or `1b`
in hex) which we will write as `'\x1b'`, followed by the `[` character. The `H`
command is short for `1;1H` which moves the cursor to line `1` and column `1`.
The `2J` command clears the entire screen. (`1J` would clear everything before
the cursor and `0J` or just `J` would clear everything after the cursor.)

Of course, this causes a problem with error handling. The screen gets cleared
after we print out the error message, so you can't see that an error occurred
unless you scroll up afterwards. We'll fix this by having `die()` call
`disableRawMode()` before printing the error message, and arranging for
`disableRawMode()` to only allow itself to execute once.

{{disabled-once}}

`disabled_once` is a static variable, which means it's basically a global
variable that can only be accessed by `disableRawMode()`. The first time
`disableRawMode()` runs, it sets `disabled_once` to `1`, which causes
`disableRawMode()` to `return` immediately if it gets called again.

That allows us to call it from `die()`, so we don't have to worry about it
getting called a second time at exit. We also need to declare
`disableRawMode()` above `die()`, since functions can only call functions that
are defined or declared above them, and these two functions call each other.
