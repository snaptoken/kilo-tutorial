...
It's harder to make `read()` fail. Perhaps if you `ssh`'d into your machine,
ran the program, and then unplugged one of the machines from the network (or
switched off wi-fi on one of them), then `read()` would fail. (_TODO_: try
this!)
