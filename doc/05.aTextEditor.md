...
changes when they try to quit, for example.
...
the `(modified)` after the filename if the file has been modified.
...
If `dirty` is set, we will display a warning in the status bar, and require the
user to press `Ctrl+Q` three more times in order to quit without saving.
...
function, which deletes the character at the given index in the given `erow`.
...
deleted character with the characters that come after it. Then we decrement the
row's `size`, call `editorUpdateRow()`, and increment `dirty`.

Now let's implement `editorDelChar()`, which uses `editorRowDelChar()` to
delete the character that's to the left of the cursor.
...
using `editorFreeRow()`. Then we use `memmove()` to overwrite the deleted row
struct with the rest of the rows that come after it, and decrement `numrows`.
Finally, we increment `dirty`.

Now let's implement `editorRowAppendString()`, which appends the given string
to the end of the given row.

{{row-append-string}}

The row's new size is `row->size + len`, so first we allocate that much memory
for `row->chars`. Then we simply `memcpy()` the given string to the end of the
contents of `row->chars`. We update `row->size`, call `editorUpdateRow()` as
usual, and increment `dirty` as usual.
...
up in the expected position afterwards.

Notice how, because moving the cursor right at the end of a line moves it to
...
We also delete the old `int at = ...` line, since that is now being passed in
as an argument.

We now have to replace all instances of `editorAppendRow(...)` with
...
`editorInsertRow()` calls `realloc()`, which might move memory around on us.
Then we truncate the current row's contents by settings its size to the
position of the cursor, and we call `editorUpdateRow()` on the truncated row.
...
a filename to save the file as. Let's make an `editorPrompt()` function that
