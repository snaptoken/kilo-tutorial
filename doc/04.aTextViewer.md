...
Now that we have scrolling, let's implement the `Page Up` and `Page Down` keys.
Like the arrow keys, these send escape sequences as input when you press them.
...
Okay, let's make `Page Up` and `Page Down` do something now.
...
`switch` statement.) Depending on which direction they want to go, we either
position the cursor at the top of the screen or the bottom. We then simulate
them pressing the Up or Down keys enough times to scroll up or down an entire
screen's worth of lines. Delegating to `editorMoveCursor()` takes care of all
the bounds-checking and cursor-fixing that needs to be done when moving the
cursor.
...
While we're at it, let's implement the `Home` and `End` keys too. Like the
...
The `Home` key could be sent as `<esc>[1~`, `<esc>[7~`, `<esc>[H`, or even
`<esc>OH`. Similarly, the `End` key could be sent as `<esc>[4~`, `<esc>[8~`,
`<esc>[F`, or `<esc>OF`. Let's handle all of these cases.

{{detect-home-end-keys}}

We also handled detecting the `Delete` key (escape sequence `<esc>[3~`), since
we will need it later and handling it now means we're finally done with the
`editorReadKey()` function, which means we're done with all the low-level
terminal functions.

Now let's make `Home` and `End` do something.

{{home-end-keys}}

`Home` brings the cursor back to the beginning of the line, and `End` brings it
to the end of the current line. If there is no current line, then `E.cx` should
be `0` and should stay at `0`, so there's nothing to do.
