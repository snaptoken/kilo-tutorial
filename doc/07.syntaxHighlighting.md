...
contents of an `erow` and highlight them by setting each value in the `hl`
array.
...
always `free()` it at the top of the function. And finally, it's impossible for
the user to edit the file between saving and restoring the `hl`, so we can
safely use `saved_hl_line` as an index into `E.row`. (It's important to think
about these things.)
...
whether the previous character was a separator. Then let's use it to highlight
numbers properly.
...
separator, or to be also be highlighted with `HL_NUMBER`.
...
code above the string highlighting code (order matters a lot in this here
function).
...
from `editorUpdateSyntax()`, which is becoming kind of a monster.
...
from their printable counterparts, we'll render them using inverted colors
(black on white).
...
You can test the coloring of non-printables by pressing `Ctrl+A`, `Ctrl+B`, and
so on to insert those control characters into strings or comments, and you
should see that they get the same color as the surrounding characters, just
inverted.
...
the file). Because `editorUpdateSyntax()` calls itself with the next line, the
change will continue to propagate to more and more lines until one of them is
unchanged, at which point we know that all the lines after that one must be
unchanged as well.

## The End

That's it! Our text editor is finished. In the [final chapter](08.theEnd.html),
I'll give you some ideas for features you might want to extend the editor with
on your own.
